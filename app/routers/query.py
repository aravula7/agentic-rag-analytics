...
async def execute_query(request: QueryRequest, background_tasks: BackgroundTasks):
    ...
    trace = langfuse_tracker.create_trace(request.query, request.user_email)
    ...
    try:
        ...
        for retry in range(settings.SQL_RETRY_MAX):
            try:
                ...
                langfuse_tracker.track_execution(trace, generated_sql, metadata, success=True)
                ...
                if retry == settings.SQL_RETRY_MAX - 1:
                    ...
                    langfuse_tracker.track_execution(
                        trace, generated_sql or "N/A", {}, success=False, error=execution_error
                    )
                    langfuse_tracker.finalize_trace(trace, success=False, error=execution_error)
        ...
        if routing_decision.get('requires_email', False) and request.user_email:
            ...
            langfuse_tracker.track_email(trace, request.user_email, success=True)
        ...
        # Finalize trace
        langfuse_tracker.finalize_trace(trace, success=True)
        ...
        langfuse_tracker.finalize_trace(trace, success=False, error=str(e))
